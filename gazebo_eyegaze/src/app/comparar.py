...
from moveit_commander.move_group import MoveGroupCommander, PlanningSceneInterface
...
GRIPPER_EFFORT = [1.0]
GRIPPER_PARAM = '/arm_1/gripper_controller'

REFERENCE_FRAME = 'base_footprint'
ARM_BASE_FRAME = 'arm_link_0'
...
class MoveItDemo:
    def __init__(self):
        ...
        self.gripper_opened = [0.011, 0.011]
        self.gripper_closed = [0 + 0.001, 0 + 0.001]
        self.gripper_neutral = [(self.gripper_opened[0] + self.gripper_closed[0])/2.0,
                                (self.gripper_opened[0] + self.gripper_closed[0])/2.0]
        
        self.gripper_tighten = 0.001
        ...
        rospy.loginfo("Set Arm: candle")
        arm.set_named_target('candle')
        if arm.go(wait=True) != True:
        ...
        n_attempts = 0
        #arm.set_joint_value_target(grasp_pose, end_effector_link, True)
        #arm.go()
        ...
        while result != MoveItErrorCodes.SUCCESS and n_attempts < max_pick_attempts:
            ...
            for grasp in grasps:
                ...
                print(grasp)
                rospy.sleep(0.2)
                result = arm.pick(target_id, grasp)
    ...
    def make_grasps(self, initial_pose_stamped, allowed_touch_objects, grasp_opening=[0]):
        ...
        g.pre_grasp_approach = self.make_gripper_translation(0.01, 0.14, [1.0, False, False])
        g.post_grasp_retreat = self.make_gripper_translation(0.01, 0.15, [False, False, 1.0])

        # Set the first grasp pose to the input pose
        g.grasp_pose = initial_pose_stamped

        # Pitch angles to try
        pitch_vals = np.linspace(pi/3, pi*2/3, num=120)
    ...
    def baseGoToGoal(self, x_goal, y_goal, theta_goal = 0.0):
        ...
        while(True):
            try:
                K_linear = 2
                distance = abs(sqrt(((x_goal - self.X[0])**2) + ((y_goal - self.X[1])**2)))
